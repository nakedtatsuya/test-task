...
export class Post {
  ...
  async save(): Promise<void> {
    ...
    const {lastID} = await db.run(
      `INSERT INTO posts (content, user_id, image_name, created_at, updated_at)
         VALUES ($content, $userId, $imageName, $createdAt, $updatedAt)`,
      {
  ...
  async update(): Promise<void> {
    ...
    const db = await databaseManager.getInstance();
    await db.run(
      "UPDATE posts SET content=$content, image_name=$imageName, updated_at=$updatedAt WHERE id=$id",
      {
        $content: this.content,
        $imageName: this.imageName,
        $updatedAt: serializeToSQLiteDateTimeString(this.updatedAt),
  ...
  static async all(): Promise<Post[]> {
    ...
    const postRowDataList = await db.all<PostRawDataWithId[]>(
      "SELECT p.id, p.content, p.user_id, p.image_name, p.created_at, p.updated_at FROM posts p ORDER BY p.created_at desc"
    );
  ...
  static async allByUser(userId: number): Promise<Post[]> {
    const db = await databaseManager.getInstance();
    const postRowDataList = await db.all<PostRawDataWithId[]>(
      "SELECT p.id, p.content, p.user_id, p.image_name, p.created_at, p.updated_at FROM posts p WHERE p.user_id=? ORDER BY p.created_at desc",
      [userId]
    );
    return postRowDataList.map(postRowData =>
      Post.fromRawDataWithId(postRowData)
  ...
  static async likedByUser(userId: number): Promise<Post[]> {
    const db = await databaseManager.getInstance();
    const postRowDataList = await db.all<PostRawDataWithId[]>(
      "SELECT p.id, p.content, p.user_id, p.image_name, p.created_at, p.updated_at FROM likes l INNER JOIN posts p ON l.post_id=p.id WHERE l.user_id=? ORDER BY p.created_at desc",
      [userId]
    );
    return postRowDataList.map(postRowData =>
      Post.fromRawDataWithId(postRowData)
  ...
  static async find(postId: number): Promise<Post | undefined> {
    ...
    const postRowData = await db.get<PostRawDataWithId>(
      "SELECT p.id, p.content, p.user_id, p.image_name, p.created_at, p.updated_at FROM posts p WHERE p.id=?",
      [postId]
    );
